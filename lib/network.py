...
class TemporalFeat(nn.Module):
    ...
    def forward(self, x, emb, pred_r_from_last, pred_t_from_last):
        # x = F.relu(self.conv1(x))
        # x = F.relu(self.conv2(x))
        ...
        x_1 = fusion = torch.cat((x, emb), dim=1)  # bs x 256 x n_p
        # print(x.shape)
        # print(emb.shape)
...
class PoseNet(nn.Module):
    def __init__(self, num_points, num_obj, object_max):
        ...
        self.test = torch.nn.Conv1d(1056, 1024, 1)
    ...
    def forward(self, img, x, choose, obj, focal_length, principal_point, motion, mask, is_train):
        ...
        out_mask = torch.gather(out_mask, 2, choose_label).contiguous()

        label_img = out_img.view(bs, di, -1)  

        x_label = torch.gather(label_img, 2, choose_label).transpose(1, 2)#emb
        ...
        if self.last_R[int(obj)] is not None:
            ...
            init_pose = torch.from_numpy(motion).cuda()
        ...
        temporal_x = self.temporal_feat(x, x_label , pred_r_from_last, pred_t_from_last)

        temporal_x = torch.cat([temporal_x, out_mask], 1)

        temporal_x = F.relu(self.test(temporal_x))
        ...
        if self.last_R_total[int(obj)] is not None:
            ...
            last_pose = torch.cat([torch.cat([self.last_R_total[int(obj)],
                                   ...
                                   torch.as_tensor([[0, 0, 0, 1]], dtype=torch.float32).repeat(1000, 1, 1).cuda()],
                                  dim=1)
            init_pose = torch.from_numpy(motion).cuda().repeat(1000, 1, 1)
            transformed_pose = torch.bmm(init_pose, last_pose)
            self.last_R_total[int(obj)] = transformed_pose[:, 0:3, 0:3]
            self.last_t_total[int(obj)] = transformed_pose[:, 0:3, 3].reshape(1, 1000, 3) - self.last_x_total[int(obj)]
